...
TEST_FOLDER = os.path.join(AUDIO_PATH, 'test_augmented')
...
def process_and_save_test_mfcc(file_paths, save_dir, n_mfcc=NUM_MFCC):
    """Process test audio files to compute and save MFCCs and infer labels."""
    data_paths = []
    labels = []
    for idx, file in enumerate(file_paths):
        y, sr = sf.read(file)
        ...
        if '_cry.wav' in file_name:
            ...
            label = 0
            label_prefix = 'notcry'
        save_path = os.path.join(save_dir, f'{label_prefix}_test_{idx}.npy')
        save_mfcc_to_disk(mfcc, save_path)
        data_paths.append(save_path)
        labels.append(label)
        del y, mfcc  # Free up memory
...
def load_test_data(file_paths, labels, num_mfcc, max_length, is_lstm):
...
def build_complex_cnn_model(hp):
    ...
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
    model.add(Dropout(rate=hp.Float('dropout_1', min_value=0.3, max_value=0.5, step=0.05)))
    ...
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
    model.add(Dropout(rate=hp.Float('dropout_2', min_value=0.3, max_value=0.5, step=0.05)))
    ...
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
    model.add(Dropout(rate=hp.Float('dropout_3', min_value=0.3, max_value=0.5, step=0.05)))

    # Fourth Convolutional Block (Larger Kernel)
    model.add(
        Conv2D(
    ...
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
    model.add(Dropout(rate=hp.Float('dropout_4', min_value=0.3, max_value=0.5, step=0.05)))
    ...
    model.add(
        Dense(
            units=hp.Int('fc_units_1', min_value=128, max_value=512, step=64),
            activation='relu',
            kernel_regularizer=l2_regularizer,
        )
    )
    model.add(Dropout(rate=hp.Float('dropout_fc1', min_value=0.3, max_value=0.5, step=0.05)))

    # Second Fully Connected Layer
    model.add(
        Dense(
            units=hp.Int('fc_units_2', min_value=64, max_value=256, step=64),
            activation='relu',
            kernel_regularizer=l2_regularizer,
        )
    )
    model.add(Dropout(rate=hp.Float('dropout_fc2', min_value=0.3, max_value=0.5, step=0.05)))
...
def convert_and_save_tflite_model(model, model_type, sample_audio_files, num_mfcc, max_length, is_lstm=False):
    """
    Convert the Keras model to TFLite format with different quantization methods and save them.
    ...
    # Convert the model to float32 TFLite
    try:
        print("Converting to float32 TFLite model...")
        converter = tf.lite.TFLiteConverter.from_keras_model(model)
        tflite_model = converter.convert()
        tflite_model_file = tflite_models_dir / f"{model_type}_cry_detection_model_float32.tflite"
        tflite_model_file.write_bytes(tflite_model)
        print(f"Float32 TFLite model saved to {tflite_model_file}")
    except Exception as e:
        print(f"Failed to convert float32 TFLite model: {e}")

    # Convert the model to float16 TFLite
    try:
        ...
        tflite_model = converter.convert()
        tflite_model_file = tflite_models_dir / f"{model_type}_cry_detection_model_float16.tflite"
        tflite_model_file.write_bytes(tflite_model)
        print(f"Float16 quantized TFLite model saved to {tflite_model_file}")
    except Exception as e:
        print(f"Failed to convert float16 quantized TFLite model: {e}")

    # Convert the model to fully quantized INT8 TFLite
    try:
        ...
        tflite_model = converter.convert()
        tflite_model_file = tflite_models_dir / f"{model_type}_cry_detection_model_int8.tflite"
        tflite_model_file.write_bytes(tflite_model)
        print(f"Fully quantized INT8 TFLite model saved to {tflite_model_file}")
    except Exception as e:
        print(f"Failed to convert fully quantized INT8 TFLite model: {e}")
...
def predict_tflite(interpreter, input_details, output_details, file_path, num_mfcc, max_length, is_lstm):
    ...
    # Quantize the input data if the model expects INT8 or float16 input
    ...
    elif output_details[0]['dtype'] == np.float16:
        output_data = output_data.astype(np.float32)
    else:
        raise ValueError(f"Unsupported output dtype: {output_details[0]['dtype']}")
...
def tflite_inference(model_filename, is_lstm=False):
    """Perform inference using the specified TFLite model."""
    # Initialize the TFLite interpreter
    interpreter = tf.lite.Interpreter(model_path=f"tflite_models/{model_filename}")
    ...
    predictions, accuracy = process_folder_tflite(interpreter, input_details, output_details, folder_path, NUM_MFCC, MAX_LENGTH, is_lstm)
    
    for file_name, prediction_label in predictions:
        print(f"File: {file_name}, Prediction: {prediction_label}")
    
    print(f"Prediction Accuracy with model {model_filename}: {accuracy:.2f}%")
...
def main():
    ...
    test_files = load_audio_files(TEST_FOLDER)

    # Process and save MFCCs for test data
    test_data_paths, test_labels = process_and_save_test_mfcc(test_files, mfcc_save_dir, n_mfcc=NUM_MFCC)

    # Load and preprocess test data
    X_test, y_test = load_test_data(test_data_paths, test_labels, NUM_MFCC, MAX_LENGTH, is_lstm=(MODEL_TYPE == 'lstm'))
    ...
    # Perform inference using TFLite models
    model_filenames = [
        f"{MODEL_TYPE}_cry_detection_model_float32.tflite",
        f"{MODEL_TYPE}_cry_detection_model_float16.tflite",
        f"{MODEL_TYPE}_cry_detection_model_int8.tflite"
    ]

    for model_filename in model_filenames:
        tflite_inference(model_filename, is_lstm=(MODEL_TYPE == 'lstm'))
